...
const TypingComponent: React.FC<{ roomId: string; userId: string }> = ({ roomId, userId }) => {
  const [userProgress, setUserProgress] = useState<Record<string, { input: string; wpm: number; isDone: boolean }>>({});
  ...
  React.useEffect(() => {
    ...
    socket.on('typing:user_progress', ({ userId, input, wpm, isDone }: TypingProgress) => {
